...
namespace FD_STOCK
{
    public partial class neemb : Form
    {
        int npr;
        ...
        private void neemb_Load(object sender, EventArgs e)
        {
            bd.Open();
        ...
        private void Enregistrer_Click(object sender, EventArgs e)
        {
            ...
            {
                bd.Open();
                SqlCommand cmd = new SqlCommand("insert into entreeemb values('" + tdee.Text + "','" + npe.Text + "','" + ddee.Value.ToString() + "')", bd);
                cmd.ExecuteNonQuery();

                SqlCommand cmd1 = new SqlCommand("select top(1) [n° entreeemb] from [entreeemb] order by [n° entreeemb] desc", bd);
                ...
                {
                    SqlCommand cmd2 = new SqlCommand("insert into dentreeemb values(" + ndee.ToString() + ",'" + tableau.Rows[i].Cells[0].Value.ToString() + "','" + tableau.Rows[i].Cells[2].Value.ToString() + "')", bd);
                    cmd2.ExecuteNonQuery();

                    SqlCommand cmd3 = new SqlCommand("select * from produitsem where [n° produitem]='" + tableau.Rows[i].Cells[0].Value.ToString() + "'", bd);
        ...
        private void Ajouter_Click(object sender, EventArgs e)
        {
            if (npro.Text != "" && int.Parse(qu.Text) > 0)
            {
                for (int i = 0; i < tableau.Rows.Count - 1; i++)
                {
                    ...
                    {
                        MessageBox.Show(" EMBALLAGE DEJA AJOUTER", "HIBA SOCKS");
                        return;
                    }
                }
                tableau.Rows.Add(
