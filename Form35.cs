...
namespace FD_STOCK
{
    public partial class om : Form
    {
        ...
        private void cmbnm_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            rd.Close();
            SqlCommand cmd1 = new SqlCommand("SELECT  ref_aiguille.reference FROM ref_aiguille INNER JOIN machine ON ref_aiguille.n_machine = machine.nm where machine.n_machine='" + cmbnm.Text + "'", bd);
        ...
        private void saveBtn_Click(object sender, EventArgs e)
        {
            try
            {
                if (cmbnm.Text != "" && ofc.Text != "" && osc.Text != "" && cmbh.Text != "")
                {
                    ...
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("ENREGISTREMENT AVEC SUCEES", "HIBA SOCKS", MessageBoxButtons.OK, MessageBoxIcon.Information);
