...
namespace FD_STOCK
{
    public partial class nf : Form
    {
        ...
        private void Remplir()
        {
            ...
            tableau.Rows.Clear();
            SqlCommand cmd = new SqlCommand("select * from fournisseur", bd);
        ...
        private void Enregistrer_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand selectCmd = new SqlCommand("SELECT * FROM fournisseur WHERE UPPER([denomination]) = @denomination", bd);
                ...
                if (rd.HasRows)
                {
                    MessageBox.Show("FOURNISSEUR DEJA EXISTE", "HIBA SOCKS", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else if (!string.IsNullOrEmpty(de.Text) && !string.IsNullOrEmpty(vil.Text))
                {
                    ...
                    SqlCommand insertCmd = new SqlCommand("INSERT INTO fournisseur VALUES (@denomination, @responsable, @ice, @telephone, @email, @siege_social, @ville)", bd);
        ...
        private void Modifier_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(de.Text))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("UPDATE fournisseur SET [denomination]=@denomination, [responsable]=@responsable, [n° ice]=@n_ice, [n° telephone]=@n_telephone, [email]=@email, [siege social]=@siege_social, [ville]=@ville WHERE [n° fournisseur]=@n_fournisseur", bd);
                    ...
                    cmd.Parameters.AddWithValue("@n_fournisseur", npr);
        ...
        private void Rechercher_Click(object sender, EventArgs e)
        {
            bd.Open();
            string input;
            input = Microsoft.VisualBasic.Interaction.InputBox("RECHERCHER N° FOURNISSEUR", "HIBA SOCKS");

            if (string.IsNullOrWhiteSpace(input))
            {
                MessageBox.Show("VEUILLEZ SAISIR N° FOURNISSEUR.", "HIBA SOCKS", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (!int.TryParse(input, out npr))
            {
                MessageBox.Show("N° FOURNISSEUR NON VALIDE.", "HIBA SOCKS", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }

            SqlCommand cmd = new SqlCommand("select * from fournisseur where [n° fournisseur]='" + npr.ToString() + "'", bd);
            SqlDataReader rd = cmd.ExecuteReader();
            if (rd.HasRows == false)
            {
                MessageBox.Show("N° FOURNISSEUR  " + npr.ToString() + " N'EXISTE PAS", "HIBA SOCKS", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                rd.Read();
        ...
        private void Supprimer_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("SUPPRIMER LE FOURNISSEUR", "HIBA SOCKS", MessageBoxButtons.YesNo, MessageBoxIcon.Stop) == DialogResult.Yes)
                {
                    bd.Open();
                    SqlCommand cmd = new SqlCommand("delete from fournisseur where [n° fournisseur]='" + npr.ToString() + "'", bd);
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("SUPPRESSION EFFECTUEE AVEC SUCCE", "HIBA SOCKS", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Nouveau_Click(sender, e);
                    Remplir();
                }
            }
        ...
        private void rec_TextChanged(object sender, EventArgs e)
        {
            ...
            bd.Open();
            tableau.Rows.Clear();
            SqlCommand cmd = new SqlCommand("select * from fournisseur where [denomination] like '" + rec.Text + "%' ", bd);
            SqlDataReader rd = cmd.ExecuteReader();
            while (rd.Read())
            {
                tableau.Rows.Add(
