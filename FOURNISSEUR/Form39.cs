        ...
        private void Enregistrer_Click(object sender, EventArgs e)
        {
            ...
            if (fo.Text != "")
            {
                ...
                for (int i = 0; i < tableau.Rows.Count - 1; i++)
                {
                    ...
                    {
                        SqlCommand cmd = new SqlCommand("UPDATE cheque SET [etat paiement] = '1' WHERE [n°cheque] = '" + tableau.Rows[i].Cells[6].Value.ToString() + "'", bd);
                        cmd.ExecuteNonQuery();
