...
namespace FD_STOCK
{
    public partial class cas : Form
    {
        ...
        private void remplir()
        {
            tableau.Rows.Clear();
            bd.Open();
            SqlCommand cmd = new SqlCommand("select * from scaisse", bd);
        ...
        private void saveBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                bd.Open();
                SqlCommand cmdCheckMontant = new SqlCommand("SELECT montant FROM caisse", bd);
                decimal montant = Convert.ToDecimal(cmdCheckMontant.ExecuteScalar());
                decimal prix = decimal.Parse(txtp.Text);
                if (montant >= prix)
                {
                    SqlCommand cmd = new SqlCommand("INSERT INTO scaisse VALUES('" + txtl.Text + "' , '" + txtp.Text + "','" + dateHour.Value + "')", bd);
                    cmd.ExecuteNonQuery();
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            saveFileDialog.FileName = "EXPORTAION SORTIE CAISSE " + DateTime.Now.ToString("yyyy_MM_dd") + ".csv";
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            tableau.Rows.Clear();
            bd.Open();
            SqlCommand cmd = new SqlCommand("select * from scaisse where libelle like '" + textBox1.Text + "%'", bd);
            SqlDataReader rd = cmd.ExecuteReader();
            while (rd.Read())
            {
                tableau.Rows.Add(
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            string query = "SELECT * FROM scaisse WHERE dh >= @fromDate AND dh <= @toDate";
