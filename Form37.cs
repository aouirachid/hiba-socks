...
namespace FD_STOCK
{
    public partial class conx : Form
    {
        ...
        {
            //  SqlDataAdapter ad = new SqlDataAdapter("select * from connexion where login = '" + logi.Text + "' and motpasse ='" + mpas.Text + "' ", bd); ; ;
            ...
            // if (dt.Rows.Count > 0)
            // {
            //  MessageBox.Show("VALIDE" ,"HIBA SOCKS");

            //  IMMATRICULATION f = new IMMATRICULATION();
            // menur f = new menur();
            // f.Show();
            // this.Hide();
            // }
            // else
            //{
            //  MessageBox.Show("NON VALIDE", "HIBA SOCKS") ;
            //}
            try
            { 
