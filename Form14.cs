...
namespace FD_STOCK
{
    public partial class lem : Form
    {
        ...
        private void rec_TextChanged(object sender, EventArgs e)
        {
            bd.Open();
            tableau.Rows.Clear();
            SqlCommand cmd = new SqlCommand("SELECT entreem.[n° entreem], produitsm.[n° referencem], entreem.[type piecem], entreem.[n° piecem], produitsm.[nom produitm], dentreem.quantitem,dentreem.[prix achathtm], produitsm.[taux tvam], entreem.[date entreem] FROM dentreem INNER JOIN produitsm ON dentreem.[n° produitm] = produitsm.[n° produitm] INNER JOIN entreem ON dentreem.[n° entrem] = entreem.[n° entreem] where produitsm.[nom produitm] like '" + rec.Text + "%' ", bd);
        ...
        private void lem_Load(object sender, EventArgs e)
        {
            bd.Open();
            Remplir();
        }
        ...
        private void exportBtn_Click(object sender, EventArgs e)
        {
            ...
            saveFileDialog.FileName = "EXPORTAION LISTE ENTREE MATIERES 1ére " + DateTime.Now.ToString("yyyy_MM_dd") + ".csv";
